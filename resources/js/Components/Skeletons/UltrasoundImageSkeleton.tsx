...
export default function UltrasoundImageSkeleton() {
    return (
        <div className="min-h-[calc(100vh-65px)] lg:h-[calc(100vh-65px)] overflow-y-auto lg:overflow-hidden flex flex-col p-3.5 sm:p-5 w-full max-w-full space-y-4 select-none pointer-events-none animate-in fade-in duration-200">
                    ...
                    <Card className="liquid-glass-card shadow-lg border border-slate-300/80 rounded-2xl flex-1 min-h-0 flex flex-col overflow-hidden max-h-none lg:max-h-[calc(100vh-140px)]">
                        ...
                        <CardContent className="p-3 space-y-2.5 flex-1 min-h-0 overflow-y-auto">
                            ...
                            {[1, 2, 3, 4].map((i) => (
                                <div
                                    key={i}
                                    className={`p-4 rounded-2xl border backdrop-blur-xl space-y-2.5 ${i === 1 ? 'bg-[#E8F8F2]/75 border-[#00875A]/40 shadow-xs' : 'bg-white/80 border-slate-200/80'}`}
                                >
                            ...
                            <div className="space-y-3.5">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                                    {Array.from({ length: 8 }).map((_, idx) => (
                                        <div
                                            key={idx}
                                            className="aspect-square bg-slate-200/80 rounded-none relative overflow-hidden border border-slate-300/60"
                                        >
